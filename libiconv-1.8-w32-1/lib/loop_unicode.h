...
/* Perry's recursive/multistep transliteration */
static int recurs_translit(conv_t cd, unsigned char *outptr, ucs4_t wc, int outleft)
{
  int ret = cd->ofuncs.xxx_wctomb(cd,outptr,wc,outleft);
  if (ret != RET_ILUNI)
    return ret;
  return unicode_transliterate(cd, wc, outptr, outleft);
}
  ...
  }
  {
    ...
    if (indx >= 0) {
      ...
      for (i = 0; i < num; i++) {
        ...
        if (cd->transliterate & 0x02) {
          /* Perry's recursive/multistep transliteration */
          sub_outcount = recurs_translit(cd,outptr,cp[i],outleft);
        } else {
